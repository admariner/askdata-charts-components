...
// Format number
function formatNumber(format, locale, value) {
...
rightPart = format.split("0");
// Firt char from the string
decimalSeparator = rightPart[1].charAt(0);
...
function getDecimals(format) {
    ...
    // Get part after decimal separator
    format.split(getDecimaltSeparator(format));
    
    // Count number of chars after the decimal separator
    var numChars = format.split(decimalSeparator)[1].length - 1;
